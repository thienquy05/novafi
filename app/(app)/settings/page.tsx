      ...
      <div className="space-y-6">
        ...
        <Card>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-start justify-between gap-4 pt-4 border-t border-slate-100">
              <div>
                <p className="text-sm font-semibold text-slate-800">{t('settings.budgetRollover')}</p>
                <p className="text-xs text-slate-500 mt-0.5">{t('settings.budgetRolloverDesc')}</p>
              </div>
              <button
                type="button"
                onClick={() => update('budgetRollover', !settings.budgetRollover)}
                className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none ${
                  settings.budgetRollover ? 'bg-indigo-600' : 'bg-slate-200'
                }`}
              >
                <span
                  className={`inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition-transform duration-200 ${
        ...
        <Card>
          ...
          {reconcileDone && (
            <div className="flex items-center gap-2 mb-4 text-sm font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">
              <CheckCircle2 className="w-4 h-4 shrink-0" />
              {t('settings.reconcileApplied')}
            </div>
          )}
          {reconcileError && (
            <div className="flex items-center gap-2 mb-4 text-sm font-bold text-rose-700 bg-rose-50 border border-rose-200 rounded-xl px-4 py-3">
              ...
              {reconcilePlan.toRepair.map((r) => (
                <div key={r.accountId} className="flex items-center justify-between gap-3 px-4 py-3 text-sm">
                  ...
                  <span className="font-semibold text-slate-500 whitespace-nowrap">
                    {formatCurrency(r.stored)} <span className="text-slate-400">→</span>{' '}
                    <span className="text-indigo-600">{formatCurrency(r.expected)}</span>
                  </span>
                </div>
              ))}
