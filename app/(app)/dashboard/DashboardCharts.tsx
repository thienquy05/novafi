    ...
    <div className="h-64 w-full mt-4">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }} barGap={6}>
          ...
          <Bar dataKey="income" name="Income" fill="url(#colorIncome)" radius={[6, 6, 0, 0]} maxBarSize={32} />
          <Bar dataKey="expenses" name="Expenses" fill="url(#colorExpense)" radius={[6, 6, 0, 0]} maxBarSize={32} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
...
export function BudgetBars({ data }: { data: BudgetData[] }) {
  if (data.length === 0) {
    return (
          ...
          >
            ...
            <div className="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden">
              <motion.div
                ...
                className={`h-full rounded-full relative ${over ? 'bg-rose-500' : pct > 80 ? 'bg-amber-500' : 'bg-indigo-500'}`}
              />
            </div>
            <p className="text-xs font-bold mt-2 text-right">
              {over ? (
                <span className="text-rose-600">{formatCurrency(Math.abs(remaining))} over budget</span>
              ) : (
...
export function NetWorthTrendChart({ data }: { data: NetWorthPoint[] }) {
  ...
  // Determine stroke and fill color based on overall trend
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`text-xs font-extrabold px-2.5 py-1 rounded-lg shrink-0 ${achieved ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-100 text-indigo-700'}`}>
                  {pct.toFixed(0)}%
                </span>
              </div>
              <div className="w-full bg-slate-200 rounded-full h-2.5 overflow-hidden">
                <motion.div
                  ...
                  animate={{ width: `${pct}%` }}
                  transition={{ duration: 1, ease: "easeOut" }}
                  className={`h-full rounded-full relative ${achieved ? 'bg-emerald-500' : 'bg-indigo-500'}`}
                />
              </div>
