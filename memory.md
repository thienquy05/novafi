...
## 2026-07-20 — Group Vault contribution silently no-op'd + funding actions didn't refresh the rest of the app (branch claude/group-vault-funding-update-fglcam)

**Request:** user reported adding money to a Group Vault didn't update the real account balance; follow-up asked for funding's quick-action buttons to trigger a platform-wide real-time update, the way `QuickAddTransaction` already does.

**Root cause of "money doesn't move" (`app/(app)/funding/page.tsx`).** `buildPoolContributionTx` (`lib/funding.ts`) intentionally returns `tx: null` — no ledger row at all — when `isMe && fromAccount === poolAccountId` (the "this money is already sitting in the vault account, just earmark it" case). That's correct in isolation, but `openContrib()` was defaulting the "Fund my share from" dropdown to the vault's own `poolAccountId` whenever it was a valid deposit account — which is true for essentially every real vault. So the single most common path (click "Add more" on your own row, type an amount, hit "Add money" without touching the pre-filled dropdown) silently hit the no-op branch: `totalContributed`/the progress bar/the milestone toast all advanced, the pool record was written, but no transaction was created, so the account's real `balance` in the Accounts sheet never changed — exactly the reported symptom. (Checked whether pool-creation's `createRealPool` also forced this — it doesn't in practice, since `draftRealParticipants()` always seeds `contributed: 0` at creation time, so `buildPoolContributionTx`'s `amt <= 0` guard short-circuits first.)

**Fix — `app/(app)/funding/page.tsx`:**
- `openContrib()`: default `contribAccount` to a deposit account **other than** `f.poolAccountId` (`depositAccounts.find(a => a.id !== f.poolAccountId)`), falling back to the vault's own account only when it's the sole deposit account available. A same-account selection is now something the user has to actively pick, not the silent default.
- Contribution modal: added an inline amber notice (`funding.fundNoTransferNote`) shown whenever `contribIsMe && contribAccount === contribFor.poolAccountId`, explicitly telling the user no money will transfer and this will only be recorded as their contribution — so picking the vault's own account on purpose is now an informed choice instead of a silent trap.
- **Locales** (`en.json`, `vi.json`): added `funding.fundNoTransferNote` (vi mirrors en).

**Fix — real-time platform update.** Neither `persist()` (the funnel for every funding mutation: create/edit/archive a pool, spend, contribution, payment, settle, migrate) nor `confirmDeletePool()` ever called `router.refresh()`, unlike `QuickAddTransaction.tsx` which does so specifically to force Next's RSC payload for other pages (Dashboard's net worth, balances, etc.) to refetch instead of serving a stale client-router-cached render. Added `useRouter()` + `router.refresh()` at the end of both `persist()`'s and `confirmDeletePool()`'s `finally` blocks, right after their existing `load(true)` (which only refreshes this page's own client state) — so any funding action now reflects everywhere else in the app immediately, matching the Quick Add pattern.

**Verification.** `npm install` (fresh env) → `tsc --noEmit` clean; `eslint` on the touched page — 0 errors (only pre-existing unused-import/`set-state-in-effect` warnings unrelated to this change); `node -e "JSON.parse(...)"` confirms both locale files still parse; `vitest run` — **616 passing** (full suite, including `lib/__tests__/funding.test.ts` and `calculations.test.ts` unaffected). No UI browser check performed (no dev server driven in this pass) — logic/typecheck/test verified only.
